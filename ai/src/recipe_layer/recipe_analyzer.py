...
from .recipe_database import RecipeDatabase, Recipe, RecipeFilter, NutritionRange
...
class RecipeAnalyzer:
    ...
    def __init__(
        ...
        """
        self.db = recipe_database or RecipeDatabase(recipe_db_path)
    ...
    def analyze_image(
        ...
        Args:
            ...
            cv_metadata: Optional CV metadata with ArUco scale
            
        Returns:
            IngredientList with detected ingredients
        """
        logger.info(f"Analyzing image for ingredients: {image_path}")
        return self.ingredient_extractor.extract_from_file(image_path, cv_metadata=cv_metadata)
