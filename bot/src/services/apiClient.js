...
/**
 * Upload a Telegram photo to the API to fulfil a pending photo request.
 * The API looks up the pending request by chatId and stores the image.
 */
export async function uploadPhotoForRequest(bot, fileId, chatId, API_URL) {
  // 1) Get file path from Telegram
  ...
  // 3) POST to /telegram/photo-upload with the chatId in the header
  const resp = await axios.post(`${API_URL}/telegram/photo-upload`, imageBuffer, {
    headers: {
